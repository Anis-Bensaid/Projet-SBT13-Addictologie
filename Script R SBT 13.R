...
# Si frqo"Jamais" FreqBinge reçoit 0, de même si binge="non". Sinon, on affecte un score allant de 1 à 5.
bdscore$FreqBinge <- ifelse(bd1$frqo=unique(bd1$frqoh)[6], 0, ifelse(bd1$binge==unique(bd1$binge)[2], 0, ifelse(is.na(bd1$frqb1),ifelse(is.na(bd1$frqb2),ifelse(is.na(bd1$frqb3), ifelse(is.na(bd1$frqb6),ifelse(is.na(bd1$frqb10),NA,5),4),3),2),1)))
...
bdscore$FreqConso <- ifelse(bd1$frqo=frqohunique[6], 0, ifelse(bd1$frqo=frqohunique[3], 1, ifelse(bd1$frqo= frqohunique[2], 2, ifelse(bd1$frqoh == frqohunique[1], 3, ifelse(bd1$frqo=frqohunique[4], 4, NA)))))
...
ACP <- PCA(full_data[-1],scale.unit=TRUE,ncp=12, grapFALSE)      
...
ACPred <- PCA(DBACP,ncp=10,scale.unit = TRUE, grapFALSE)
...
# Si nb.clust=0 et grapTRUE, la fonction affiche le dendrograme à l'aide d'une console grâce à 
